...
if (isset($_SESSION["userId"])) {
  ...
  function getReceiverName($dbConnect, $receiverId)
  {
    ...
    $senderName->bind_param("s", $receiverId);
    $senderName->execute();
  ...
  function renderFirstView($fullName, $appointmentDate, $appointmentTime, $eventName, $location, $appointmentStatus, $appointmentId)
  {
    return
      ...
      <td>
      <a href='delete-sent.php?id={$appointmentId}' class='btn btn-secondary btn-sm rounded-0'>Delete</a>
      </td>
      </tr>";
  }
  ...
  $appointmentDetailsQuery = "SELECT * FROM appointment_details WHERE sender_id = ? ORDER BY appointment_date ASC";
  $appointmentDetails = $dbConnect->prepare($appointmentDetailsQuery);
  $appointmentDetails->bind_param('s', $senderId);
  ...
  if ($appointmentDetailsResult->num_rows > 0) {
    while ($appointmentDetailsrow = $appointmentDetailsResult->fetch_assoc()) {
      $receiverName = getReceiverName($dbConnect, $appointmentDetailsrow["receiver_id"]);
      $fullName = htmlspecialchars(strtoupper($receiverName["first_name"] . " " . $receiverName["last_name"]), ENT_QUOTES, "UTF-8");
